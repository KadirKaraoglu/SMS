  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      if (!token || !storedUsername || !storedRole) {
        ...
        navigate(`/login/academician`);
        return;
      }

      try {
        const res = await fetch(`http://localhost:8080/api/teacher/me`, {
        ...
        if (!res.ok) {
          ...
          try {
            ...
            console.warn(
              "Backend response for user info is not JSON:",
              errorText
            );
          }
          ...
          if (errorMessage === "Your session has timed out") {
            ...
            navigate("/login/academician");
          } else if (errorMessage === "The teacher could not find !") {
            toast.error("User information not found. Please contact support.");
          } else {
            toast.error(`Failed to load user info: ${errorMessage}`);
          }
      ...
      <div className={styles.main}>
        ...
        <div className={styles.middle}>
          ...
          <div style={{ color: "rgb(77, 77, 113)", fontSize: "17px" }}>
            <p>
              **Name, Surname and Lecture changes are made by your Department
              Secretary.**
            </p>
          </div>
...
export default UserInformationTeacher;
